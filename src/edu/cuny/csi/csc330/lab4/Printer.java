...
public class Printer {
	...
	public static final String ERROR_PRINTER_OFF = "Maybe try turning it on first?";
	public static final String ERROR_PRINTER_UNPLUG = "Maybe try plugging it in first?";
	...
	public void togglePower() {
		if (outletState) {
			...
			System.out.println("Maybe try plugging it in first?");
		}
	}
	...
	public void loadPaper(int paperAmt) {
		if((paperCount + paperAmt) > PAPER_MAX) {
			System.out.println("Too much paper! " + (paperCount+paperAmt - PAPER_MAX) + " pieces of paper fell out as you pushed the paper tray in...");
	...
	public void loadInk(int inkAmt) {
		if ((inkLevel + inkAmt) > INK_MAX) {
			System.out.println("Too much ink! " + (inkLevel+inkAmt - INK_MAX) + " ink amount spilled...");
			inkLevel = INK_MAX;
		}
		else {
			inkLevel += inkAmt;
	...
	public void processJobs() {
		...
		while (!jobQueue.isEmpty() ) {
			...
			if (paperCount < intHolder) {
				...
				System.out.println("No/not enough paper! Please reload and then continue job.");
				interrupted = true;
				busyState = false;
				break;
				...
				System.out.println("No/not enough ink! Please reload and then continue job.");
				interrupted = true;
				busyState = false;
				break;
			} else {
...
public static void main(String[] args) {
	...
	if (argsInt.length < 1) {
		System.err.println("Enter >1 args");
	} else {
